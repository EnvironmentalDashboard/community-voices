<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/package">
    <div style="display: flex;align-items:center;">
      <img src="http://environmentaldashboard.org/cv/uploads/{domain/slide/image/image/id}" alt="{domain/slide/image/image/title}" style="flex-shrink: 0;width: auto;height: 86vh;" />
      <h1 style="color:#fff;font-family:Comfortaa, sans-serif;padding:10px;font-size:3vw;">
    ...
    <div>
      <xsl:choose>
        <xsl:when test="domain/slide/contentCategory/contentCategory/id = 1">
          <xsl:attribute name="style">width:100%;background:rgb(150,81,23);position:absolute;bottom:0;height:14vh</xsl:attribute>
        </xsl:when>
        <xsl:when test="domain/slide/contentCategory/contentCategory/id = 2">
          <xsl:attribute name="style">width:100%;background:rgb(92,92,92);position:absolute;bottom:0;height:14vh</xsl:attribute>
        </xsl:when>
        <xsl:when test="domain/slide/contentCategory/contentCategory/id = 3">
          <xsl:attribute name="style">width:100%;background:rgb(4,54,75);position:absolute;bottom:0;height:14vh</xsl:attribute>
        </xsl:when>
        <xsl:when test="domain/slide/contentCategory/contentCategory/id = 4">
          <xsl:attribute name="style">width:100%;background:rgb(86,114,34);position:absolute;bottom:0;height:14vh</xsl:attribute>
        </xsl:when>
        <xsl:when test="domain/slide/contentCategory/contentCategory/id = 5">
          <xsl:attribute name="style">width:100%;background:rgb(67,118,45);position:absolute;bottom:0;height:14vh</xsl:attribute>
        </xsl:when>
        <xsl:when test="domain/slide/contentCategory/contentCategory/id = 6">
          <xsl:attribute name="style">width:100%;background:rgb(94,0,224);position:absolute;bottom:0;height:14vh</xsl:attribute>
        </xsl:when>
      </xsl:choose>
      <img src="http://via.placeholder.com/200x200?text=Content Category" alt="" style="position:absolute;right:3vw;bottom:4vw;width:10vw;height:auto">
        <xsl:choose>
          <xsl:when test="domain/slide/contentCategory/contentCategory/id = 1">
